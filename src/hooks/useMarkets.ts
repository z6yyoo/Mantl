...
import { fetchPolymarketViaProxy } from '@/lib/polymarket'
...
export function useMarkets(platform: Platform | 'all' = 'all') {
  ...
  const fetchAll = useCallback(async () => {
    ...
    // Filter: 30-60% probability range for depth analysis
    const filtered = results.filter(m => {
      if (m.status !== 'active') return false
      const leadPrice = m.outcomes[0]?.price ?? 0.5
      return leadPrice >= 0.30 && leadPrice <= 0.60
    })
  ...
  useEffect(() => {
    const initialId = setTimeout(fetchAll, 0)
