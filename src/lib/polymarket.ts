...
function getPolymarketPrice(m: Record<string, unknown>): number {
  ...
  if (rawPrices && rawPrices !== '[]') {
    try {
      ...
      const yesPrice = Number.parseFloat(p[0] || '0')
...
export function normalizePolymarketEvents(events: PolymarketEvent[]): Market[] {
  const markets: Market[] = []

  for (const event of events) {
    if (!event.markets || event.markets.length === 0) continue
    const hasOpenMarkets = event.markets.some((m) => !m.closed)
    ...
    try {
      ...
      if (event.markets.length > 1) {
        const activeSubMarkets = event.markets.filter((m) => !m.closed)
        const marketsToUse = activeSubMarkets.length > 0 ? activeSubMarkets : event.markets

        outcomes = marketsToUse.map((m) => {
...
export async function fetchPolymarketViaProxy(limit: number, offset: number): Promise<Market[]> {
  ...
  return normalizePolymarketEvents(events)
}
